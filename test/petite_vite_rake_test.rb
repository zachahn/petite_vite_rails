...
class PetiteViteRakeTest < ActiveSupport::TestCase
  RAKE_PATH = File.expand_path("../lib/tasks/petite_vite.rake", __dir__)
  ...
  def test_defines_vite_build_and_vite_place_tasks
    ...
    assert Rake::Task.task_defined?("petite_vite:build")
    assert Rake::Task.task_defined?("petite_vite:place")
  end
  ...
  def test_enhances_assets_precompile_when_defined
    ...
    assert_equal ["petite_vite:build", "petite_vite:place"], Rake::Task["assets:precompile"].prerequisites
  end
  ...
  def test_enhances_test_prepare_when_defined
    ...
    assert_equal ["petite_vite:build", "petite_vite:place"], Rake::Task["test:prepare"].prerequisites
  end
