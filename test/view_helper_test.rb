...
class PetiteViteViewHelperTest < ActionView::TestCase
  ...
  def test_vite_tags_in_development_emits_the_vite_client_and_the_frontend_output
    Rails.env.stub(:development?, true) do
      tag = vite_tags

      assert tag.html_safe?
      assert_includes tag, '<script type="module" src="http://localhost:5173/@vite/client"></script>'
      assert_includes tag, '<script type="module" src="http://localhost:5173/src/main.js"></script>'
    end
  end

  def test_vite_tags_in_development_uses_configured_vite_dev_server_port
    shared_json_path = File.join(@tmpdir, "petite_vite_with_port.json")
    File.write(shared_json_path, JSON.dump({
      "buildCommand" => "yarn build",
      "viteDevServerPort" => 6000,
      "frontendOutput" => "/src/main.js",
    ...
    Rails.env.stub(:development?, true) do
      ...
      assert_includes tag, '<script type="module" src="http://localhost:6000/@vite/client"></script>'
      assert_includes tag, '<script type="module" src="http://localhost:6000/src/main.js"></script>'
    end
  end
  ...
  def test_vite_tags_in_production_emits_stylesheets_module_script_and_modulepreloads_for_entries
    Rails.env.stub(:development?, false) do
      ...
      assert tag.html_safe?
      assert_includes tag, '<link rel="stylesheet" href="/assets/main-abc123.css" />'
      assert_includes tag, '<link rel="stylesheet" href="/assets/lazy-def456.css" />'
      assert_includes tag, '<script type="module" src="/assets/main-abc123.js"></script>'
      assert_includes tag, '<link rel="modulepreload" href="/assets/lazy-def456.js" />'
    end
  end
