...
module PetiteViteRails
  module E2EHarness
    ...
    def run_install_generator(cell, workdir, vite_dev_server_port:, rails_port:)
      bin_dev = File.join(workdir, "bin/dev")
      File.unlink(bin_dev) if File.exist?(bin_dev)
      args = [
        "bundle", "exec", "rails", "g", "petite_vite:install",
        "--vite-dev-server-port", vite_dev_server_port.to_s,
    ...
    def assert_install_artifacts(test, cell, workdir)
      ...
      test.assert_includes File.read(vite_config_path), "shared.viteCorsOrigin"
